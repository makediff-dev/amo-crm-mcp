...
export const getLeadNotesInputSchema = z.object({
  lead_id: z.number().int().positive()
    .describe('Lead ID to get notes for'),
...
export const addNoteInputSchema = z.object({
  lead_id: z.number().int().positive()
    .describe('Lead ID to add note to'),
  text: z.string().min(1).max(10000)
    .describe('Note text content'),
  note_type: noteTypeEnum.default('common').optional()
    .describe('Тип записи: common (обычная), sms_in (входящая SMS), sms_out (исходящая SMS). По умолчанию: common')
});

export const updateNoteInputSchema = z.object({
  lead_id: z.number().int().positive()
    .describe('Lead ID the note belongs to'),
  note_id: z.number().int().positive()
    .describe('Note ID to update'),
  text: z.string().min(1).max(10000)
    .describe('New text content for the note')
});
